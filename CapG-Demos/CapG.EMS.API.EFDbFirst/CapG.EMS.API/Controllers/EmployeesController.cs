...
namespace CapG.EMS.API.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        [HttpPost]
        public ActionResult Post(Employees employees)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    bool isAdded = repository.Add(employees);
                    if (isAdded)
                    {
                        return Created("employee", employees);
                    }
                    else
        ...
        [HttpPut("{id}")]
        public ActionResult Put(int id, Employees employees)
        {
            if (ModelState.IsValid)
            {
                if (id == employees.Id)
                {
                    try
                    {
                        bool isUpdated = repository.Update(employees);
                        if (isUpdated)
