            ...
            if (double.TryParse(input, out double num1))
            {
                ...
                if (double.TryParse(input, out double num2))
                {
                    ...
                    if (int.TryParse(input, out int choice))
                    {
                        ...
                        {
                            case 1:
                                double sum = MathOps.Add(num1, num2);
                                Console.WriteLine("Sum = " + sum);
                                break;
                            case 2:
                                double diff = MathOps.Subtract(num1, num2);
                                Console.WriteLine("Difference = " + diff);
                                break;
                            case 3:
                                double pro = MathOps.Multiply(num1, num2);
                                Console.WriteLine("Product = " + pro);
                                break;
                            case 4:
                                double quo = MathOps.Divide(num1, num2);
